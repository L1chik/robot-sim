            ...
            match event.value {
                ...
                WindowEvent::CursorPos(x, y, modif) => {
                    if modif.contains(CTRL) {
                        ...
                        let move_gain = 0.005_f32;
                        rotate_joint(
                            robot.get_active_joint(),
                ...
                WindowEvent::Key(input, Action::Press, _modif) => match input {
                    Key::Key1 => robot.set_joint_index(0),
                    Key::Key2 => robot.set_joint_index(1),
                    Key::Key3 => robot.set_joint_index(2),
                    Key::Key4 => robot.set_joint_index(3),
                    Key::Key5 => robot.set_joint_index(4),
                    Key::Key6 => robot.set_joint_index(5),
                    Key::Key7 => robot.set_joint_index(6),
                    Key::J => rotate_joint(robot.get_active_joint(), -0.017, robot),
                    Key::U => rotate_joint(robot.get_active_joint(), 0.017, robot),
                    Key::P => {
